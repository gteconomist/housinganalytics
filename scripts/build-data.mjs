...
// Excel rows are 1-indexed; the array is 0-indexed.
// Row 12 (ACS codes) is index 11.
const headerRow = rows[11];
if (!headerRow) throw new Error('Could not find ACS-code header row (Excel row 12).');
...
// Find the special columns (Area Name and GeoID/FIPS) from row 12 of the spec.
//   Col B (index 1) = 'NAME'
//   Col C (index 2) = 'Geo_geoid_'
const NAME_COL  = headerRow.indexOf('NAME');
const GEOID_COL = headerRow.indexOf('Geo_geoid_');
if (NAME_COL < 0 || GEOID_COL < 0) {
  throw new Error('Could not locate NAME / Geo_geoid_ columns in spreadsheet row 12.');
}
...
const counties = [];
for (let r = 12; r < rows.length; r++) {            // data starts at Excel row 13 → index 12
  const row = rows[r];
  if (!row) continue;
