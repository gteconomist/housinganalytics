...
// Places data (incorporated cities, towns, villages, boroughs ≥5,000 pop).
//
// Reads "Full Housing Data Table - Places.xlsx" — the static annual pull
// generated by scripts/build-places-xlsx.mjs. ACS only refreshes once a year,
// so pulling at build time is wasteful (used to take ~7 minutes via the
// Census API). Re-run build-places-xlsx.mjs each December when the new
// ACS 5-year vintage drops.
//
// Returns the same shape that fetch-acs-places.mjs used to produce, so the
// downstream place-processing code below is unchanged.
async function loadPlaces() {
  const f = join(__dirname, '..', 'Full Housing Data Table - Places.xlsx');
  try {
    ...
    // Find header row (same approach as the counties parser).
    let hdrIdx = -1;
    for (let i = 0; i < Math.min(rows.length, 50); i++) {
      const r = rows[i];
      if (r && r.includes('NAME') && r.includes('Geo_geoid_')) { hdrIdx = i; break; }
    }
    if (hdrIdx < 0) {
      console.warn('Places XLSX: could not find header row — places will be empty.');
      return { places: {}, place_count: 0 };
    }
    const hdr = rows[hdrIdx];
    const nameCol   = hdr.indexOf('NAME');
    const geoidCol  = hdr.indexOf('Geo_geoid_');
    const parentCol = hdr.indexOf('parent_county_fips');

    // Vintage banner: row 5 col C in the counties workbook; locate dynamically.
    let vintageYear = null;
    for (let i = 0; i < Math.min(rows.length, 12); i++) {
      const r = rows[i] || [];
      for (let c = 0; c < r.length; c++) {
        const v = r[c];
        if (typeof v === 'number' && v >= 2010 && v <= 2050) { vintageYear = v; break; }
      }
      if (vintageYear) break;
    }
    ...
    for (let r = hdrIdx + 1; r < rows.length; r++) {
      ...
      let geoid = row[geoidCol];
      if (!name || geoid == null) continue;
      geoid = String(geoid).padStart(7, '0');
      const stateFips = geoid.slice(0, 2);
      const placeFips = geoid.slice(2);
      let parent = parentCol >= 0 ? row[parentCol] : null;
      if (parent != null && parent !== '') parent = String(parent).padStart(5, '0');
      else parent = null;
      ...
      for (let c = 0; c < hdr.length; c++) {
        const code = hdr[c];
        if (!code) continue;
        if (code === 'NAME' || code === 'Geo_geoid_' || code === 'parent_county_fips') continue;
        const v = row[c];
        if (v == null || v === '') { vars[code] = null; continue; }
        const n = typeof v === 'number' ? v : Number(v);
        if (!Number.isFinite(n) || n < -1_000_000) { vars[code] = null; continue; }
        vars[code] = n;
      }
      places[geoid] = {
        geoid, name,
    ...
    return {
      ...
      vintage: vintageYear ? `ACS 5-year ${vintageYear}` : null,
      acs_year: vintageYear,
      min_population: 5000,  // matches build-places-xlsx.mjs default
    };
  } catch (e) {
    console.warn(`loadPlaces: ${e.message} — places will be empty.`);
    return { places: {}, place_count: 0 };
  }
}
...
for (const [geoid, raw] of Object.entries(placesRaw.places ?? {})) {
  ...
  // A handful of Census place names contain their own comma — e.g. Florida's
  // "Islamorada, Village of Islands village, Florida" — so split on the LAST
  // comma rather than assuming a clean two-part name. The state is always
  // the final piece; everything before it is the place name.
  const parts = String(raw.name).split(',').map(s => s.trim()).filter(Boolean);
