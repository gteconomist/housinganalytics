...
function buildSheet(tab, model) {
  ...
  const cols = 2 + Math.max(...TABS.flatMap((t) => t.sections.map((s) => (s.fields ? s.fields.length : 0) * (vintages.length + 1))), 8);
  ...
  for (const section of tab.sections) {
    ...
    // single-vintage shares. Different shape from the field-column layout below.
    if (section.kind === 'matrix') {
      const yr = section.vintage || vintages[vintages.length - 1];
      push([S(`${section.title}  (${yr})`)]);
      // header: Geography | Age Cohort | <bucket labels>
      push(['', S('Geography'), S('Age Cohort'), ...section.buckets.map((b) => S(b.label))]);
      const emitMatrixGeo = (key) => {
        const r = records[key];
        section.cohorts.forEach((coh, ci) => {
          const arr = ['', ci === 0 ? S(r.label) : '', S(coh.label)];
          section.buckets.forEach((_b, j) => {
            const v = r.byVintage[yr] ? r.byVintage[yr][`incage_${coh.id}_${j}`] : null;
            arr.push(N(v, '0.0%'));
          });
          push(arr);
        });
      ...
      contextKeys.forEach(emitMatrixGeo);
      push([]);
      push([S('  Comparison communities')]);
      peerKeys.forEach(emitMatrixGeo);
      push([]);
...
function buildNotes(model) {
  const notes = [
    ...
    ['  6. Income by Age Cohort (B19037): each value is the share of that age cohort\'s households in the income band (rows sum to 100%). Shown for the latest vintage only.'],
    [],
    ['Not included in this core-ACS cut (deferred)'],
    ['  HUD FMR & CHAS cost burden; LEHD OnTheMap commuting; Woods & Poole / ARC / Georgia-OPB forecasts; SchoolDigger; Zillow; crime.'],
    [],
